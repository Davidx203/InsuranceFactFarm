...
//  TipFarm
//
//  Created by Luis Delgado on 9/16/2023
import Foundation
import WidgetKit
...
class ContentViewModels: ObservableObject {
  ...
  // List of tips
  let tips = [
    "The dedutible is what you pay when an accident occurs, before your policy covers the loss.",
  ...
  // Populate tipAndQuestionDictionary with random tips and questions, distributing them across days
    public func provideTipsAndQuestionsForCurrentWeek() -> [Int: (tip: String, questions: [String], choices: [String], correctChoice: String)]{
        self.nextDay()
    for day in 1...7 {
        let randomTipIndex = Int.random(in: 0..<tips.count)
        let randomTip = tips[randomTipIndex]
      ...
      return tipAndQuestionDictionary
  }
